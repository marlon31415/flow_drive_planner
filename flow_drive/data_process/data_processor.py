...
from flow_drive.data_process.ego_process import (
    ...
    get_ego_past_array_from_history_buffer,
...
class DataProcessor(object):
    def __init__(self, config):
        ...
        self.max_ped_bike = (
            10  # Limit the number of pedestrians and bicycles in the agent.
        )
    ...
    def observation_adapter(
        ...
        ego_agent_past, time_stamps_past = get_ego_past_array_from_history_buffer(
            history_buffer, self.num_past_poses
        )
        ...
        """
        observation_buffer = (
            history_buffer.observation_buffer
        ...
        ego_agent_past, neighbor_agents_past, _, static_objects = agent_past_process(
            ego_agent_past,
    ...
    def exclude_existing_scenarios(self, scenarios, saved_npz_files):
        ...
        filtered_scenarios = [
            ...
            if (scen._map_name, scen.token) not in existing_scenarios
        ]
    ...
    def work_multiprocess(self, scenarios, n_processes):
        ...
        for i in range(n_processes):
            p = Process(
                target=self._worker,
                args=(
                    list(scenarios_per_process[i]),
                    i,
                ),  # Pass scenario chunk and process_id
                name=f"worker_{i}",
            )
            p.daemon = (
                True  # Daemonize processes so they exit when the main program exits
            )
            p.start()
            processes.append(p)
    ...
    def _worker(self, scenarios_chunk, process_id: int):
        ...
        for scenario in tqdm(
            scenarios_chunk, desc=f"{process_name}", position=process_id, leave=True
        ):
            try:
                self.process_scenario(scenario)
                ...
                print(
                    f"[{process_name}] Error on {scenario.token if hasattr(scenario, 'token') else 'unknown scenario'}: {e}"
                )
