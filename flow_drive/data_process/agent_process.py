        ...
        for j in range(agents[i].shape[0]):
            if target_array.shape[0] > 0:
                ...
                is_in_target_frame: bool = bool(
                    (
                        agent_id == target_array[:, AgentInternalIndex.track_token()]
                    ).max()
                )
...
def agent_past_process(
    ...
    if len(ped_bike_indices) + len(vehicle_indices) <= num_agents:
        ...
        selected_indices = sorted(
            selected_indices, key=lambda idx: distance_to_ego[idx]
        )[:num_agents]

    # Populate the final agents array with the selected agents' features
    for i, j in enumerate(selected_indices):
        agents[i, :, : agents_array.shape[-1]] = agents_array[
            :, j, : agents_array.shape[-1]
        ]
    ...
    static_objects = np.zeros(
        (num_static, static_objects_array.shape[-1] + 4), dtype=np.float32
    )
